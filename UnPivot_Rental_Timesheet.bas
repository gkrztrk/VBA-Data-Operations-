Attribute VB_Name = "UnPivot_Rental_Timesheet"
Option Explicit
...
'Loop through each Excel file in folder
  Do While myFile <> ""
    ...
    'Change First Worksheet's Background Fill Blue
      Call UnpivotWorkbook(wb_data, ws_data)
...
Sub UnpivotWorkbook(wbData As Workbook, wsData As Worksheet)
    ...
    For Each ws In ActiveWorkbook.Worksheets
        If ws.Name <> "DATA" Then
            ...
            'If ws.Tab.ColorIndex <> 4 And ws.Visible Then
                Call UnpivotWorksheets(wbData, wsData)
...
End Sub
Sub UnpivotWorksheets(wbData As Workbook, ws As Worksheet)
...
Set bulunan = ActiveSheet.Range(Cells(1, 1), Cells(50, 50)).Find("SERIAL NO.")
Set ss = ActiveSheet
...
Set clnfind = ActiveSheet.Range(Cells(1, 1), Cells(50, 50)).Find("SERIAL NO.")
...
Set clnfind = ActiveSheet.Range(Cells(1, 1), Cells(50, 50)).Find("OF EQUIPMENT")
...
Set clnfind = ActiveSheet.Range(Cells(1, 1), Cells(50, 50)).Find("PROJECT CODE")

If Not clnfind Is Nothing Then

    prjct_code = clnfind.Column
    
End If
    
Set clnfind = ActiveSheet.Range(Cells(1, 1), Cells(50, 50)).Find("ZONE")
...
For j = date_column To (ss.Cells(date_row, Columns.Count).End(xlToLeft).Column) - 1
    date_column = j
    lr = bulunan.Row + 1
    ...
    For i = lr To ss.Cells(Rows.Count, 1).End(xlUp).Row
        ...
        'ws.Cells(dr, 1) = dr - 1
        ws.Cells(dr, 2) = ss.Cells(date_row, date_column) + ThisWorkbook.Worksheets("UnPivot PMV EQ Timesheets").Range("B6") - 1
       ...
       End If
        ...
        '-------------------------------------------------------
        If ss.Cells(3, 3).Value <> "" Then
            ws.Cells(dr, 12) = ss.Cells(3, 3).Value
            
        Else
        
            ws.Cells(dr, 12) = ActiveWorkbook.Name
